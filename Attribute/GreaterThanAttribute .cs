...
namespace jQueryAjaxInAsp.NETMVC.Attribute
{
    public sealed class GreaterThanAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            // Compare values
            if (Int32.Parse(value.ToString()) >= Int32.Parse(propertyTestedValue.ToString()))
            {
                if (this.allowEqualDates && value == propertyTestedValue)
                {
                    return ValidationResult.Success;
                }
                else if (Int32.Parse(value.ToString()) > Int32.Parse(propertyTestedValue.ToString()))
                {
                    return ValidationResult.Success;
                }
            }
